...
public class NoaaSource implements ForecastSource {
	...
	private static final String TYPE_12HOUR = "12 hour";
	 ...
	 */
	private static List<Forecast> flattenForecasts(
		...
		for (String layout : forecasts.keySet()) {
			...
			for (Forecast forecast : forecasts.get(layout)) {
				if (!TextUtils.isEmpty(forecast.getConditions())) {
					flat.add(forecast);
				} else if (forecast.getMorningPrecip() != Integer.MIN_VALUE) {
					Forecast realForecast = forecasts.get("k-p24h-n5-1").get(
							index / 2);
					if (index % 2 == 0) {
						realForecast.setMorningPrecip(forecast
								.getMorningPrecip());
					} else {
						realForecast.setEveningPrecip(forecast
								.getMorningPrecip());
					}
				}
				++index;
	...
	public List<Forecast> getForecasts(double lat, double lon, int days)
		...
		if (Double.isNaN(lat) || Double.isNaN(lon)) {
			...
			Log.d(TAG, String.format(
					"queryLocation() with lat=%f, lon=%f, days=%d", lat, lon,
					days));
		}
		...
		try {
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				if (eventType == XmlPullParser.START_TAG) {
					...
					if (TAG_TEMPERATURE.equals(thisTag)
							|| TAG_WEATHER.equals(thisTag)
							|| TAG_POP.equals(thisTag)
							|| TAG_HAZARDS.equals(thisTag)) {
						...
						Forecast forecast = getForecast(forecasts, thisLayout,
								++index);
						forecast.setConditions(xpp.getAttributeValue(null,
					...
					if (TAG_LAYOUT_KEY.equals(thisTag)) {
						...
						Forecast forecast = getForecast(forecasts, thisLayout,
								++index);
						forecast.setValidStart(parseDate(xpp.getText()));

					} else if (TAG_VALUE.equals(thisTag)
							&& TYPE_MAXIMUM.equals(thisType)) {
						Forecast forecast = getForecast(forecasts, thisLayout,
								++index);
						forecast.setTempHigh(Integer.parseInt(xpp.getText()));
						forecast.setUrl(detailsUrl);

					} else if (TAG_VALUE.equals(thisTag)
							&& TYPE_MINIMUM.equals(thisType)) {
						Forecast forecast = getForecast(forecasts, thisLayout,
								++index);
						forecast.setTempLow(Integer.parseInt(xpp.getText()));

					} else if (TAG_VALUE.equals(thisTag)
							&& TYPE_12HOUR.equals(thisType)) {
						Forecast forecast = getForecast(forecasts, thisLayout,
								++index);
						forecast.setMorningPrecip(Integer.parseInt(xpp
								.getText()));

					} else if (TAG_HAZARDTEXTURL.equals(thisTag)) {
						Forecast forecast = getForecast(forecasts, thisLayout,
								index);
						forecast.setUrl(xpp.getText());
