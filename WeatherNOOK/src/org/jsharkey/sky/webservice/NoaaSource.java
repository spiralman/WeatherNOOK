        ...
        for (String layout : forecasts.keySet()) {
            ...
            for (Forecast forecast : forecasts.get(layout)) {
                if (!TextUtils.isEmpty(forecast.getConditions())) {
                	...
                	Forecast realForecast = forecasts.get("k-p24h-n5-1").get(index / 2);
                	if(index % 2 == 0) {
                		realForecast.setMorningPrecip(forecast.getMorningPrecip());
                	} else {
                		realForecast.setEveningPrecip(forecast.getMorningPrecip());
                	}
                }
                ++index;
            }
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (TAG_TEMPERATURE.equals(thisTag) || TAG_WEATHER.equals(thisTag)
                        ...
                        forecast.setConditions(xpp.getAttributeValue(null, ATTR_WEATHER_SUMMARY));
                    ...
                    } else if (TAG_VALUE.equals(thisTag) && TYPE_12HOUR.equals(thisType)) {
                        Forecast forecast = getForecast(forecasts, thisLayout, ++index);
                        forecast.setMorningPrecip(Integer.parseInt(xpp.getText()));
