...
public class LocationRetrieval {
	...
	public List<ForecastLocation> getLocations(String searchString)
		...
		String result = WebserviceHelper
				.readApi(String
		...
		try {
			...
			for (int i = 0; i < results.length(); ++i) {
				...
				JSONArray addressComponents = address
				...
				double latitude = address.getJSONObject("geometry")
						.getJSONObject("location").getDouble("lat");
				double longitude = address.getJSONObject("geometry")
						.getJSONObject("location").getDouble("lng");

				ObservationStation closestStation = m_stationDB
						.getClosestStation(latitude, longitude);

				locations.add(new ForecastLocation(city, state, fullName,
						latitude, longitude, closestStation));
			}
		} catch (JSONException e) {
			throw new ParseException("Error parsing Location results: "
					+ e.getMessage());
		}
