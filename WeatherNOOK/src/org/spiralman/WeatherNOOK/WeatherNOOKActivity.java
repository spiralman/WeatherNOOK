...
public class WeatherNOOKActivity extends ActionBarActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		View current = getLayoutInflater().inflate(R.layout.current_conditions,
				null);
		...
		ObservationStationDB stationDB = new ObservationStationDB(
				WeatherNOOKActivity.this);
		stationDB.open();

		Object configInstance = getLastNonConfigurationInstance();

		// TODO: This has gotten complex enough it should really be a state
		// machine (along with onStart()).
	...
	public void onStart() {
		...
		if (m_locationRetrieval != null && m_location != null) {
			if (m_report != null) {
				Date now = new Date();
				Date refreshAfter = m_report.getCurrentConditions()
						.getRefreshAfter();

				if (!m_didRestoreReport && refreshAfter != null
						&& now.after(refreshAfter)) {
					refresh();
				} else {
					displayReport();
				}
	...
	private void displayReport() {
		...
		SimpleAdapter adapter = new SimpleAdapter(WeatherNOOKActivity.this,
				forecastMap, R.layout.forecast_entry,
				new String[] { "Forecast" }, new int[] { R.id.forecastLayout });
		...
		conditionImage.setImageResource(ForecastUtils.getIconForForecast(
				conditions.getConditions(), isDaytime));
		conditionLabel.setText(conditions.getConditions());
		moreInfoLabel
				.setText(Html.fromHtml(String.format(
						"<a href=\"%1$s\">%2$s</a>", m_report.getUrl(),
						m_moreInfoText)));
		tempLabel.setText(String.format(m_tempFormat,
				Math.round(conditions.getTemperature())));
		windLabel.setText(String.format(m_windFormat,
				Math.round(conditions.getWindSpeed()),
				conditions.getWindDirAbbreviation()));

		if (conditions.getHumidity() >= 0) {
			humidityLabel.setVisibility(View.VISIBLE);
			humidityLabel.setText(String.format(m_humidityFormat,
		...
		updatedLabel.setText(String.format(m_updatedFormat,
				conditions.getObservationTime()));
	...
	private AlertDialog createConfigDialog() {
		...
		final EditText location = (EditText) content
				.findViewById(R.id.location);
		final ListView resultsList = (ListView) content
			...
			public void onClick(View v) {
				try {
					...
					if (!TextUtils.isEmpty(locationText)) {
						List<ForecastLocation> locations = m_locationRetrieval
								.getLocations(locationText);

						resultsList
								.setAdapter(new ArrayAdapter<ForecastLocation>(
										WeatherNOOKActivity.this,
										android.R.layout.select_dialog_singlechoice,
										locations
												.toArray(new ForecastLocation[0])));
			...
			public void onClick(DialogInterface dialog, int which) {
				try {
					...
					if (selectedPos != ListView.INVALID_POSITION) {
						m_location = (ForecastLocation) resultsList
								.getAdapter().getItem(selectedPos);
						Log.d("WeatherNOOK",
								"Selected location: " + m_location.toString());
						Log.d("WeatherNOOK", "Current Condition URL: "
								+ m_location.getObservationStation().getUrl());
	...
	private class LocationInitializeThread extends
			BlockingAsyncTask<ObservationStationDB, Void, LocationRetrieval> {
		...
		public LocationRetrieval doInBackground(
				ObservationStationDB... stationDBs) {
			...
			try {
				...
				stationDB.importStations(new InputStreamReader(assets
						.open("noaa_weather_station_index.xml")));
		...
		public void notifyComplete(LocationRetrieval result) {
			...
			if (m_exception == null) {
				...
				m_currentActivity.promptException(
						"Error initializing Weather Station Database:",
						m_exception);
			}
	...
	private class RefreshThread extends
			BlockingAsyncTask<Void, Void, WeatherReport> {
		Exception m_exception = null;

		public RefreshThread(WeatherNOOKActivity activity) {
			super(activity);
			setRefreshMessage(String.format("Loading Forecast for %s...",
					activity.m_location.toString()));
		}
		...
		protected void notifyComplete(WeatherReport report) {
			...
			if (m_exception == null) {
				...
				m_currentActivity.promptException(
						"Error loading weather report:", m_exception);
			}
